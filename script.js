...
// script.js (with backend integration)

// const API_BASE = 'https://backend-vz89.onrender.com/api';
// const calendar = document.getElementById('calendar');
// const monthYear = document.getElementById('monthYear');
// const modal = document.getElementById('taskModal');
// const cancelBtn = document.getElementById('cancelModal');
...
//   dayDiv.innerHTML += `<div class="date">${formatted}</div>`;

//   const tasks = await getTasksForDate(dateStr);
//   tasks.forEach(task => {
//     const badge = document.createElement('div');
//     badge.className = 'task-badge';
//     badge.textContent = `${task.start}-${task.end} ${task.popis} ${task.znacka}`;
//     dayDiv.appendChild(badge);
//   });
...
//   try {
//     const res = await fetch(`${API_BASE}/tasks?date=${dateStr}`);
...
//   const task = {
//     date: dateStr,
//     popis: taskForm.popis.value,
//     customPopis: taskForm.customPopis?.value || '',
//     znacka: taskForm.znacka.value,
//     poistovna: taskForm.poistovna.value,
//     start: taskForm.startTime.value,
//     end: taskForm.endTime.value,
...
//   if (!task.popis || !task.znacka || !task.start || !task.end || !task.meno || !task.telefon) {
...
// script.js (full backend integration with GET/POST/DELETE for tasks and holidays)
...
async function renderDayCell(dateStr) {
  ...
  tasks.forEach(task => {
    ...
    badge.className = 'task-badge';
    badge.textContent = `${task.start}-${task.end} ${task.popis} ${task.znacka}`;
    badge.addEventListener('click', () => deleteTask(task._id));
    dayDiv.appendChild(badge);
  });
...
holidayForm?.addEventListener('submit', async function (e) {
  ...
  const holiday = {
    ...
    to: holidayForm.holidayTo.value,
    type: holidayForm.holidayType.value,
    notes: holidayForm.holidayNotesText?.value || ''
  };
  ...
  try {
    ...
    holidays.forEach(holiday => {
      ...
      if (fromDate.getMonth() === month && fromDate.getFullYear() === year) {
        const div = document.createElement('div');
        div.className = 'holiday-item';
        div.innerHTML = `${holiday.holidayEmployee}: ${fromDate.toLocaleDateString('sk-SK')} - ${toDate.toLocaleDateString('sk-SK')} (${holiday.type})<br><small>${holiday.notes}</small>`;
        const btn = document.createElement('button');
        btn.textContent = '🗑️';
        btn.addEventListener('click', () => deleteHoliday(holiday._id));
        div.appendChild(btn);
        holidayList.appendChild(div);
      }
    });
...
async function deleteHoliday(id) {
  if (!confirm('Naozaj chcete vymazať túto dovolenku?')) return;
  try {
